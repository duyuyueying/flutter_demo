...
class GridNav extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return PhysicalModel(
      ...
      child: Column(children: _griNavItems(context))
    );

  }

  _griNavItems(BuildContext context) {
    List<Widget> items = [];
  ...
  _gridNavItem(BuildContext context, GridNavItemModel gridNavItem, bool first) {
    ...
    Color endColr = Color(int.parse('0xff'+gridNavItem.endColor));
    return Container(
      height: 88,
      margin: first?null:EdgeInsets.only(top: 3),
      decoration: BoxDecoration(
        gradient: LinearGradient(colors: [startColor, endColr])
      ),
  ...
  _mainItem(BuildContext context, CommonModel model) {
    return _wrapGesture(context, GestureDetector(
      child: Stack(
